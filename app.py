...
archivo = client.open("registro_horas_brian")
...
def calcular_horas(row):
    try:
        ...
        # 👇 FORMATO CLAVE (12 horas con am/pm)
        e = pd.to_datetime(
            f"{fecha} {row['Entrada']}",
            format="%Y-%m-%d %I:%M %p",
            errors="coerce"
...
for i, t in enumerate(trabajos):
    with cols[i % 3]:
        if st.button(t, key=f"btn_{i}"):
            ...
            st.session_state.confirmar_borrado = False  # reset seguro

# =============================
# ELIMINAR TRABAJO (SEGURO)
# =============================
...
if st.session_state.trabajo_activo:
    ...
    tipo = st.radio(
        "Inicio de semana",
        ["Lunes a domingo", "Sábado a viernes"]
    )
    ...
    if "key_activa" not in st.session_state or st.session_state.key_activa != key:
        st.session_state.key_activa = key
...
if st.button("📋 Ver semana guardada"):
    semana_guardada = cargar_semana_guardada(
        st.session_state.trabajo_activo,
        tipo,
        inicio_str
    )

    if semana_guardada is None:
        st.warning("⚠️ No hay semana guardada aún")
    else:
        st.subheader("📄 Semana guardada")
        st.dataframe(
            semana_guardada,
            use_container_width=True
        )
        st.success(
            f"🟢 Total guardado: {round(semana_guardada['Horas'].sum(),2)} h"
        )
