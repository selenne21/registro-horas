...
def calcular_horas(row):
    try:
        ...
        e = pd.to_datetime(f"{fecha} {row['Entrada']}",
                           format="%Y-%m-%d %I:%M %p", errors="coerce")
        s = pd.to_datetime(f"{fecha} {row['Salida']}",
                           format="%Y-%m-%d %I:%M %p", errors="coerce")
        ...
        if row["Inicio break"] and row["Fin break"]:
            b1 = pd.to_datetime(f"{fecha} {row['Inicio break']}",
                                format="%Y-%m-%d %I:%M %p", errors="coerce")
            b2 = pd.to_datetime(f"{fecha} {row['Fin break']}",
                                format="%Y-%m-%d %I:%M %p", errors="coerce")
...
for i, t in enumerate(trabajos):
    with cols[i % 3]:
        if st.button(t, key=f"trabajo_{i}"):
